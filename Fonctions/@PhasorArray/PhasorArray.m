classdef PhasorArray  < matlab.mixin.indexing.RedefinesParen & matlab.mixin.indexing.RedefinesBrace & matlab.mixin.CustomDisplay
    ...
    methods
        ...
        function [Eew,E] = ACenergy(o1,elementwise)
            ...
            aco1 = o1.extract(1:o1.h);
            Eew = sum(aco1.value .* conj(aco1.value), 3);
            E = sum(Eew, 'all');
            if nargout<2
                if ~elementwise
                    Eew = E;
        ...
        function [Eew,E] = DCenergy(o1,elementwise)
            ...
            dco1 = o1.extract(0);
            Eew = sum(dco1.value .* conj(dco1.value), 3);
            E = sum(Eew, 'all');
            if nargout<2
                if ~elementwise
                    Eew = E;
        ...
        function r = mrdivide(o1,o2,optarg)
        ...
        function r = mldivide(o1,o2,optarg)
        ...
        function [r,residual] = mlHmcDivide_old(A, B, optarg)
            ...
            arguments
                ...
                B PhasorArray
                optarg.h                = []
                optarg.thresholdResidual = 5e-4 %0.05% error
            ...
            % Solve T_tb(A, hOut, hIn) * F_tb(X, hIn) = F_tb(B, hOut)
            % This is an overdetermined system (rectangular Toeplitz)
            res_tb = A.spTB(hOut, hIn) \ B.spF_tb(hOut);

            % Reconstruct PhasorArray using hIn dimensions
            ...
            if optarg.autoUpdateh
                ...
                residual.h_history        = hIn;
                residual.regime_history        = {};
                residual.s_alg_history        = [];
                residual.s_exp_history        = [];
                r_best           = r;
                ...
                while resrelnorm_best > optarg.thresholdResidual && hIn < maxhIn
                    % Determine next hIn step
                    spectral_width = (A.h + B.h);
                    if hIn < spectral_width * 1.1 || strcmp(optarg.updateMethod,'incremental')
                        regime = 'initial';
                        hIn = hIn + 1;
                    else
                        % Log-linear jump once asymptotic regime is reached
                        idx_start = find(residual.h_history >= spectral_width * 1.1, 1);
                        if isempty(idx_start) || idx_start == numel(residual.h_history)
                            regime = 'initial';
                            hIn = hIn + 1;
                        else
                            % Regime Detection: Semi-log vs Log-log
                            idx1 = idx_start;
                            idx2 = numel(residual.h_history);
                            e1 = residual.resrel_history(idx1);
                            e2 = residual.resrel_history(idx2);
                            h1 = residual.h_history(idx1);
                            h2 = residual.h_history(idx2);

                            % 1. Exponential decay: log(e) = s_exp * h + c
                            s_exp = (log(e2 + eps) - log(e1 + eps)) / (h2 - h1 + eps);
                            % 2. Algebraic decay: log(e) = s_alg * log(h) + c
                            s_alg = (log(e2 + eps) - log(e1 + eps)) / (log(h2 + eps) - log(h1 + eps));
                            % Predict next h for both models
                            target = optarg.thresholdResidual;
                            h_exp = h2 + ceil((log(target + eps) - log(e2 + eps)) / (s_exp - eps));
                            h_alg = ceil(h2 * (target / (e2 + eps))^(1 / (s_alg - eps)));

                            % Decision: Select regime based on predicted target feasibility
                            % If algebraic slope is shallow (e.g. > -1), it's likely non-invertible
                            if s_alg < -0.1 && s_alg > -1.5
                                % Likely algebraic decay (non-invertible)
                                deltah = h_alg - h2;
                                regime = 'algebraic';
                            elseif s_exp < -1e-4
                                % Exponential decay
                            ...
                            residual.regime_history{end+1} = regime;
                            residual.s_alg_history(end+1) = s_alg;
                            residual.s_exp_history(end+1) = s_exp;

                            % Apply safety factors and clamping
                            deltah = ceil(deltah * 0.8);
                            deltah = max(1, deltah);
                            deltah = min(deltah, 50);            % Max absolute jump
                            deltah = min(deltah, ceil(hIn * 0.5)); % Max 50% increase
                            hIn = h2 + deltah;

                            if optarg.verbose > 1
                                fprintf('mlHmcDivide: %s regime, s_exp=%.2e, s_alg=%.2f, predicted h_target=%d, jump=+%d\n', ...
                                    regime, s_exp, s_alg, min(h_exp, h_alg), deltah);
                            end

                            if strcmp(regime, 'algebraic') && h_alg > maxhIn
                                % Early exit if target is clearly unattainable
                                ...
                                if h2 > last_warn_h + 100
                                    warning('phasorArray:mlHmcDivide:slowConvergence', ...
                                        'Slow algebraic convergence detected (slope %.2f). Target order predicted at h=%d (exceeds maxhIn=%d).', ...
                                        s_alg, h_alg, maxhIn);
                                    last_warn_h = h2;
                                end
                            else
                                algebraic_hit_count = 0; % Reset if regime changes
                            end
                        end
                    end
                    ...
                    if numel(residual.resrel_history) >= stagnationWindow
                        ...
                        if relative_improvement < stagnationRatio
                            warning('phasorArray:mlHmcDivide:stagnation', ...
                                ['mlHmcDivide: residual stagnated at %e after hIn=%d ' ...
                                '(< %.0f%% improvement over %d steps).\n' ...
                                '  Returning best solution found (hIn=%d, relative residual norm=%e).'], ...
                                residual.resnorm, hIn, stagnationRatio*100, stagnationWindow, ...
                                (size(r_best.value,3)-1)/2, resrelnorm_best)
                            r = r_best;
                ...
                if hIn >= maxhIn && residual.resrelnorm > optarg.thresholdResidual
                    warning('phasorArray:mlHmcDivide:maxhReached', ...
                        ['mlHmcDivide: reached maxh=%d without convergence (relative residual=%e).\n' ...
                        '  Returning best solution found (relative resnorm=%e).'], ...
                        maxhIn, residual.resrelnorm, resrelnorm_best)
                    r = r_best;
                    residual.resnorm = resnorm_best;
                    residual.resrelnorm = resrelnorm_best;
                end
                ...
                if optarg.plotConvergence && isfield(residual, 'h_history')
                    figure;
                    tiledlayout(3,1);
                    
                    % 1. Log Residual
                    nexttile;
                    plot(residual.h_history, log10(residual.resrel_history + eps), '--o');
                    grid on; title('log10(Relative Residual)'); xlabel('h');
                    
                    if ~isempty(residual.s_alg_history)
                        % Match lengths: histories are recorded only once asymptotic regime starts
                        h_slopes = residual.h_history(end-numel(residual.s_alg_history)+1 : end);
                        
                        % 2. Algebraic Slope
                        nexttile;
                        plot(h_slopes, residual.s_alg_history, '--o');
                        grid on; title('Algebraic Slope (s_{alg})'); xlabel('h');
                        
                        % 3. Exponential Slope
                        nexttile;
                        plot(h_slopes, residual.s_exp_history, '--o');
                        grid on; title('Exponential Slope (s_{exp})'); xlabel('h');
                    end
                end
            else
            if residual.resrelnorm > optarg.thresholdResidual
                warning('phasorArray:mlHmcDivide:residual', ...
                    'mlHmcDivide: relative residual norm is %e (threshold: %e), consider increasing h or using autoUpdateh=true.', ...
                    residual.resrelnorm, optarg.thresholdResidual)
            end
        end
        if issquare(r)
            residual.ressym =norm(value(r-r')/2,'fro');
            residual.resasym =norm(value(r+r')/2,'fro');
        end
    ...
    function [oInv, oInvt, norm_err, norm_ref] = inv(o1, optarg)
        ...
        arguments
            ...
            optarg.m               = []
            optarg.plot            = false
            optarg.reduceThreshold = 4e-15
            optarg.reduceMethod    = 'relative'
            optarg.autoTrunc       = false
            optarg.verbose         = false
            optarg.evalInv         = false
        end
    ...
    function r = BT(o1, m, h2)
        ...
        %
        %   See also: TB, spBT, array2BToepliz.
        arguments
            o1
            m = []
            h2 = []
    ...
    function [res,residual] = lyap_old(o1,o2,o3,optarg)
        ...
        if isempty(o3)
            ...
            %convert to sylvester form
            o3 = o2; %Q becomes C
            o2 = o1; %A becomes B
            o1 = o1.'; %A becomes A.'
        else
            %lyap(A,B,C,h,T) : syvester equation dot M + A(t) M + MB(t) + C = 0 truncated to h harmonics
            h = optarg.h;
            T = optarg.T;
        ...
        if optarg.autoUpdateh
            ...
            while residual.resrelnorm > optarg.thresholdResidual && h < maxh
                h = h + 1;
                res = PhasorArray(SylvHarmonic(o1,o2,o3,h,2*pi/T));
                residual.phasor  = res.d(T) + o1*res + res*o2 + o3;
                residual.resnorm = norm(residual.phasor.value, 'fro');
                ...
                if numel(residual.resrel_history) >= stagnationWindow
                    ...
                    if relative_improvement < stagnationRatio
                        warning('phasorArray:lyap:stagnation', ...
                            ['lyap: relative residual stagnated at %e after h=%d ' ...
                            '(< %.0f%% improvement over %d steps).\n' ...
                            '  System may be ill-conditioned or h is insufficient.\n' ...
                            '  Returning best solution found (h=%d, relative resnorm=%e).'], ...
                            residual.resrelnorm, h, stagnationRatio*100, stagnationWindow, ...
                            (size(res_best.value,3)-1)/2, resrelnorm_best)
                        res = res_best;
                        residual.resnorm = resnorm_best;
                        residual.resrelnorm = resrelnorm_best;
                        break
            ...
            if h >= maxh && residual.resrelnorm > optarg.thresholdResidual
                warning('phasorArray:lyap:maxhReached', ...
                    ['lyap: reached maxh=%d without convergence (relative residual=%e).\n' ...
                    '  Returning best solution found (relative resnorm=%e).'], ...
                    maxh, residual.resrelnorm, resrelnorm_best)
                res = res_best;
                residual.resnorm = resnorm_best;
                residual.resrelnorm = resrelnorm_best;
            end
            ...
            if optarg.verbose
                fprintf('lyap: solved for h=%d, relative residual=%e\n', ...
                    (size(res.value,3)-1)/2, residual.resrelnorm)
            end
        else
            if residual.resrelnorm > optarg.thresholdResidual
                warning('phasorArray:lyap:residual', ...
                    'lyap: relative residual norm is %e (threshold: %e), consider increasing h or using autoUpdateh=true.', ...
                    residual.resrelnorm, optarg.thresholdResidual)
            end
        end
        if issquare(res)
            residual.resPsym =norm(value(res-res'),'fro');
        end
